...
if [ "$1" == "--backup" ];then
    docker ps -a|awk '{print $1}' 2>/dev/null|| echo "Choose from the container in the list ..."
...
if [ "$1" == "--savetofile" ];then
   docker images|awk '{print $1}' 2>/dev/null|| echo "Choose from the image in the list ..."
   echo "Usage: ./mrclean.sh --backup "
   echo "Enter BackupName Savename:"
   read savename backupname
   docker save -o ./$savename.tar $backupname
fi

if [ "$1" == "--restore" ];then
  docker images|awk '{print $1}' 2>/dev/null|| echo "Choose from the image in the list ..."
  echo "Usage: ./mrclean.sh --restore"
  echo "Enter Tar FileName:"
  read filename
  docker load -i $filename 2>/dev/null|| echo "Choose from the image in the list ..."
fi 	
