...
if [ "$1" == "--frombackup" ];then
    docker volume create --name nexus-data 2>/dev/null
    docker  run --name $2 -d -p 8081:8081 -v nexus-data:/nexus-data $(docker images|egrep -v 'REPOSITORY|sonatype/nexus3'|awk '{print $1}') | echo "Creating docker image from backup"
    exit 0
else
    $(docker ps -a|grep $2 |awk '{print $13}') 2>/dev/null | echo "Container has already been created.."
fi
